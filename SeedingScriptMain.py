...
SCRIPT_CONFIG_SETTINGS_FILE = os.path.abspath(f'{SCRIPT_CONFIG_SETTINGS_FOLDER}/seedingconfig.json')
GAME_CONFIG_PATH = os.path.abspath(f"{LOCAL_APPDATA}/SquadGame/Saved/Config/WindowsNoEditor")
ICONS_PATH = os.path.abspath(f'{SCRIPT_CONFIG_SETTINGS_FOLDER}/icons')
    ...
    left_col = sg.Column([
        [sg.Frame('', layout=[
            ...
            [sg.InputText(size=(18, 20), key='-SERVER_IP-', default_text=server_ip, enable_events=True),
             sg.InputText(size=(5, 10), key='-PLAYER_THRESHOLD-', default_text=player_threshold, enable_events=True,
                          pad=(55, 0))],
            ...
            [sg.Frame('Threshold of players', [
             ...
             sg.Frame("", layout=[
                 ...
                 [sg.InputText(key='-ATTEMPTS_TO_AUTOJOIN-', size=(5, 5), default_text=attempts_to_autojoin,
                               enable_events=True,
                 ...
                 [sg.InputText(key='-SLEEP_INTERVAL-', size=(5, 5), default_text=sleep_interval, enable_events=True,
                               tooltip=
                 ...
                 [sg.InputText(key='-GAME_START_DELAY-', size=(5, 5), default_text=game_start_to_autojoin_delay,
                               tooltip=
    ...
    right_col = sg.Column([
        [sg.Frame('', size=(400, 900), layout=[
            ...
            [sg.InputText(size=(35, 20), key='-GAME_INSTALL-',
                          default_text=squad_install_path, enable_events=True),
            ...
            [sg.InputText(size=(35, 20), key='-SERVER_HANDLE-', default_text=server_handle_to_autojoin,
                          enable_events=True)],
    ...
    seedingscript_config = \
        {
            ...
            'game_start_to_autojoin_delay': {
                'value': 20,
                'description': 'The delay from when the game was launched by the script,'
                               ' to when the script will attempt to perform an autojoin'
...
def find_resolution(game_config_path):
    """
...
def main():
    ...
    init_config_folder(SCRIPT_CONFIG_SETTINGS_FOLDER)
    # Creates the config file if one doesen't exist, in the user's Appdata folder
    init_JSON_config(SCRIPT_CONFIG_SETTINGS_FILE)
